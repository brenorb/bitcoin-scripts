...
Every hostile input is range-checked before the numeric equality scans. The
target is validated at script-generation time and must be in `0..=15`. The
...
terminal predicate, unrelated live state, and transaction context. The 33-byte
witness figure is the canonical 16-item profile, not a maximum over accepted
nonminimal encodings. No hints are required.
