//! Batched population-count projection for canonical u4 limbs.
...
/// Consume canonical nibbles and replace each with its Hamming weight.
///
/// Before: `preserved | nibble[0] | ... | nibble[n-1]`.
/// After: `preserved | weight[0] | ... | weight[n-1]`.
...
mod tests {
    ...
    #[test]
    fn projects_all_nibble_population_counts_in_order() {
        let result = execute_script(script! {
            { u4_hex_to_nibbles("0123456789abcdef") }
            { u4_nibbles_to_popcount(16) }
            for nibble in (0..16u32).rev() {
                { popcount(nibble) } OP_EQUALVERIFY
            }
            OP_TRUE
        });
        assert!(
            result.success,
            "population-count projection failed: {result}"
        );
    }
