...
bytes must compose the existing `verify_canonical_nibble()` boundary before this
classifier.

The representative configuration uses threshold `5` over 16 canonical one-byte
witness nibbles. It includes range checks, two numeric comparisons, branch selection,
