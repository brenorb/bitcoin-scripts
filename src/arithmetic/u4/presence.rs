    ...
    fn compares_nonminimal_numeric_encodings_by_value() {
        ...
        for alias in [vec![1, 0]] {
            for alias_index in 0..3 {
                ...
                .expect("presence execution with nonminimal encoding");
                assert!(
                    result.success,
                    "wrong presence bits for alias {alias:?} at {alias_index}: {result}"
                );
            }
        }
        ...
        for alias in [vec![0x80], vec![0, 0]] {
            for alias_index in 0..3 {
                ...
                .expect("presence execution with zero alias");
                assert!(
                    result.success,
                    "wrong presence bits for zero alias {alias:?} at {alias_index}: {result}"
                );
            }
        }
    }
