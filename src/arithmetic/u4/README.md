...
- `parity::u4_nibbles_to_parity(nibble_count)` takes a checked batch size in
  `1..=982`.
- `popcount::u4_nibbles_to_popcount(nibble_count)` takes a checked batch size in
  `1..=982` and returns each nibble's Hamming weight.
...
| Checked population-count batch, 32 nibbles | <!-- metric:u4_popcount_batch32 -->440<!-- /metric:u4_popcount_batch32 --> bytes | <!-- metric:u4_popcount_batch32_stack -->50<!-- /metric:u4_popcount_batch32_stack --> items | <!-- metric:u4_popcount_batch32_opcodes -->328<!-- /metric:u4_popcount_batch32_opcodes --> |
...
<!-- metric:u4_popcount_batch32_witness -->65<!-- /metric:u4_popcount_batch32_witness --> serialized witness bytes for the representative population-count batch.
...
The population-count table uses the same 16-item checked lookup schedule and
returns the number of set bits in each nibble (`0..=4`). It is useful when a
consumer needs Hamming weights rather than parity or a four-bit expansion.
...
Population count uses the same numeric range proof before its `OP_PICK` lookup.
Its output is a ScriptNum in `0..=4`, not a terminal predicate.
...
live state.
For `u4_nibbles_to_popcount(n)`, the same input ordering is consumed and
replaced one-for-one by Hamming weights. The standalone peak is `n + 18` during
range checks; the generator rejects `n > 982`, and callers must reduce the
batch for unrelated live state.
...
`popcount.rs` exhaustively check the 16-value lookup domain, reject malformed
